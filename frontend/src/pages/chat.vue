        ...
        <div class="header">
            ...
            <div class="user-settings">
                ...
                <div class="detail-change">
                    Change Color
                    <div class="colors">
                        <div v-for="color in colors" :class="['color', color.value, { selected: color.isSelected }]"
                            :data-color="color.value" @click="changeColor(color)" :key="color.value"></div>
                    </div>
                </div>
                <div class="settings">
                ...
                <img class="user-profile" :src="dataUserInfomation.url">
            </div>
        </div>
        <div class="wrapper">
            <div class="conversation-area">
                <div v-for="user in userList" :key="user.id" @click="selectUser(user)">
                    <div v-if="user.id !== currentUserID" class="msg online">
                        <img class="msg-profile" :src="user.url" alt="" />
            ...
            <div class="chat-area" v-if="selectedUser">
                <div class="chat-area-header">
                    <div class="chat-area-title">{{ selectedUser.username }}</div>
                    <div class="chat-area-group">
                        <img class="chat-area-profile" :src="selectedUser.url" alt="" />
                    </div>
                </div>
                <div class="chat-area-main">
                    <div class="chat-msg" v-for="message in messages" :key="message.id">
                        {{ message.content }}
                    </div>
                </div>
                ...
                <div class="chat-area-footer">
                    ...
                    <form @submit.prevent="sendMessage" style="width: 100%;">
                        <input v-model="inputMessage" type="text" placeholder="Type something here..." />
                    </form>
                    <div style="display: inline-flex; margin-left: 2%;">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            ...
            <div class="detail-area">
                ...
                <div class="detail-changes">
                    ...
                    <div class="detail-change">
                        Change Color
                        <div v-for="color in colors" :class="['color', color, { selected: selectedColor === color }]"
                            :data-color="color" @click="changeColor(color)" :key="color"></div>
                    </div>
                    <div class="detail-change">
                        Change Emoji
                        <svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 24 24" fill="none" stroke="currentColor"
        ...
        methods:
        {
            ...
            sendMessage() {
                if (this.inputMessage) {
                    const message = {
                        ...
                        id_sender: this.currentUserID,
                        receiver: this.selectedUser.username,
                        id_receiver: this.selectedUser.id
                    };

                    // Gửi tin nhắn tới máy chủ
                    this.socket.emit('chat message', message);

                    // Kiểm tra xem người nhận tin nhắn có phải là người đang được chọn không
                    if (this.selectedUser && message.id_receiver === this.selectedUser.id) {
            ...
            Messenger() {
                ...
                this.socket.on('chat message', (message) => {
                    ...
                    // Kiểm tra xem người gửi tin nhắn có phải là người đang được chọn không
                    if (this.selectedUser && message.id_sender === this.selectedUser.id) {
                        // Cuộn đến cuối danh sách tin nhắn
                        this.$nextTick(() => {
                            const messagesContainer = document.querySelector('.chat-area');
                            messagesContainer.scrollTop = messagesContainer.scrollHeight;
                ...
                this.socket.on('chatHistory', (chatHistory) => {
                    this.messages = chatHistory.map((entry) => {
                        return {
                            id: entry.Message.id,
            ...
            async getUserInformation() {
                try {
                    const response = await this.$axios.get(`getUserById/${this.currentUserID}`);
                    const userInformation = response.data;
                    this.dataUserInfomation = userInformation;
                    // Điều gì bạn muốn làm với thông tin người dùng ở đây? Bạn có thể gán nó vào một biến trong data để hiển thị trong template hoặc thực hiện bất kỳ xử lý nào khác.
                    console.log(this.dataUserInfomation);
                } catch (error) {
                    console.error(error);
                }
            },
